...
class PSO_Generalized:
    def __init__(self, num_particles, dimensions, max_iter, fitness_values,particles_position):
        ...
        # Definindo limites
        self.MinPosition_limit = [1.0, 8.0, 1.0, 8.0, 1.0, 16.0, 1.0, 0.0001, 16.0]
        self.MaxPosition_limit = [9.0, 32.0, 3.0, 32.0, 3.0, 64.0, 32.0, 0.01, 128.0]
        self.velocity=[]
...
for datasetNumber in range(1,21):
    ...
    pso_generalized = PSO_Generalized(num_particles=num_particles, dimensions=9, max_iter=30,fitness_values=fitness_values,particles_position=particles_position)
    ...
    with open(trainingFile, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([dataTreatment.decimalToName(int(round(best_position[0]))), int(round(best_position[1])), int(round(best_position[2])), int(round(best_position[3])), int(round(best_position[4])), int(round(best_position[5])), int(round(best_position[6])), best_position[7], int(round(best_position[8])), best_fitness])
        file.close()
    print("Saved")